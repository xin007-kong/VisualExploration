...
def draw_huffman_tree(root):
    ...
    while queue:
        ...
        labels[node.index] = node.value if node.value else node.index
        if parent is not None:
            G.add_edge(parent.index, node.index)
        if node.left:
    ...
    nx.draw(G, pos, labels=labels, with_labels=True, node_size=3000, node_color='skyblue', font_size=15, width=2, edge_color='gray', font_weight='bold')
    plt.title('Huffman Tree', size=20)
    plt.show()
